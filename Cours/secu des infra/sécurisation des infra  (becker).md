...
N'oubliez pas que le répertoire des cybercriminels est vaste et en constante évolution. Parfois, ils peuvent combiner deux ou plusieurs des tactiques ci-dessus pour augmenter leurs chances.
...
**Le faux-semblant consiste à utiliser la tromperie pour créer un scénario visant à convaincre les victimes de divulguer des informations qu'elles ne devraient pas divulguer. Les prétextes sont souvent utilisés contre les entreprises qui conservent des données sur les clients, telles que des données financières, des numéros de carte bancaire, des numéros de compte de services publics et d’autres informations sensibles. Les usurpateurs demandent souvent des informations à des personnes dans une entreprise en usurpant l'identité d'un superviseur, d'un employé du service d'assistance ou d'un client, généralement par téléphone, par e-mail ou par SMS.**
...
**WEP (Confidentialité équivalente filaire)** et **WPA (Accès protégé sans fil)** sont des protocoles de sécurité conçus pour sécuriser les réseaux sans fil vulnérables aux attaques.
...
Pour les cyberpirates, toutes les cibles se valent. Ils peuvent aussi bien attaquer les terminaux vulnérables de particuliers que de PME ou d'entreprises plus importantes, qu'elles soient publiques ou privées.
...
Les navigateurs Internet sont utilisés par tout le monde (ou presque). Impossible d'envisager de bloquer complètement la navigation sur Internet, car les entreprises ont besoin d'accéder au web sans porter atteinte à la sécurité de celui-ci.
...
Toutes les attaques ne sont pas menées côté serveur. Les attaques par scripts intersites (XSS) le cybercriminel injecte un script malveillant dans les pages web exécutées côté client depuis son propre navigateur web. Ces scripts peuvent être utilisés par un langage (Visual Basic, JavaScript, etc.) pour accéder à un ordinateur, collecter des informations sensibles, lancer de nouvelles attaques ou diffuser des malwares. À l'instar de l'injection de code SQL, cette attaque est souvent le fruit d'une publication de contenu par le hacker sur un site de confiance qui ne valide pas les entrées. Les visiteurs du site web de confiance sont alors exposés à du contenu publié par le hacker.

Il existe deux types principaux de scripts XSS:

- **Stocké (persistant) -** Il est stocké en permanence sur le serveur infecté et est reçu par tous les visiteurs de la page infectée.
- **Réfléchi (non persistant) -** Il suffit que le script malveillant se trouve dans un lien et que les visiteurs doivent cliquer sur le lien infecté pour être infectés.

Voici certaines solutions permettant de prévenir ou de réduire les attaques XSS:

- Assurez vous que les développeurs d'applications Web sont conscients des vulnérabilités XSS et de la façon de les éviter.
- Implémenter un système IPS pour détecter et prévenir les scripts malveillants.
- Utilisez un proxy Web pour bloquer les sites malveillants.
- Utiliser un service tel que Cisco Umbrella pour empêcher les utilisateurs d'accéder à des sites web connus comme étant malveillants.
- Comme pour toutes les autres mesures de sécurité, veillez à former vos utilisateurs finaux. Apprenez - leur à identifier les attaques par phishing et à avertir le personnel Infosec lorsqu'ils suspectent un incident lié à la sécurité.
...
Une vigilance constante et une formation continue sont nécessaires pour protéger votre réseau contre les attaques. Les bonnes pratiques à suivre pour sécuriser un réseau sont les suivantes:

- Élaborer une politique de sécurité écrite pour l’entreprise.
- Sensibilisez les employés aux risques de l'ingénierie sociale et élaborez des stratégies pour valider les identités par téléphone, par courriel ou en personne.
- Contrôler l'accès physique aux systèmes.
- Utilisez des mots de passe forts et changez-les souvent.
- Chiffrez et protégez les données sensibles par mot de passe.
- Mettez en œuvre du matériel et des logiciels de sécurité tels que des pare-feu, des systèmes de prévention des intrusions (IPS), des périphériques de réseau privé virtuel (VPN), des logiciels antivirus et le filtrage du contenu.
- Effectuez des sauvegardes et testez régulièrement les fichiers sauvegardés.
- Arrêtez les services et les ports inutiles.
- Mettez à jour les correctifs en les installant chaque semaine ou tous les jours, si possible, pour éviter les attaques par débordement de la mémoire tampon et par élévation des privilèges.
- Effectuez des audits de sécurité pour tester le réseau.



# Limitation des malwares

Les malwares, notamment les virus, les vers et les chevaux de Troie, peuvent causer de graves problèmes sur les réseaux et les terminaux. Les administrateurs réseau disposent de plusieurs moyens pour limiter ces attaques.

**Remarque:** les techniques d'atténuation sont souvent appelées « contre-mesures » dans la communauté des spécialistes de la sécurité.

Un logiciel antivirus est un moyen d'atténuer les attaques de virus et de chevaux de Troie. Les logiciels antivirus empêchent les hôtes d'être infectés et de propager du code malveillant. Il faut beaucoup plus de temps pour nettoyer les ordinateurs infectés que pour maintenir à jour les logiciels antivirus et les définitions antivirus sur les mêmes machines.

Le logiciel antivirus est le produit de sécurité le plus largement déployé sur le marché aujourd'hui. Plusieurs entreprises qui créent des logiciels antivirus, comme Symantec, McAfee et Trend Micro, détectent et éliminent les virus depuis plus de dix ans. De nombreuses entreprises et établissements d'enseignement achètent des licences en volume pour leurs utilisateurs. Les utilisateurs peuvent se connecter à un site web avec leur compte et télécharger le logiciel antivirus sur leurs postes de travail, ordinateurs portables ou serveurs.

Les produits antivirus disposent d'options d'automatisation des mises à jour qui permettent de télécharger automatiquement ou à la demande les nouvelles définitions de virus et les nouvelles mises à jour logicielles. Cette pratique est essentielle pour protéger un réseau contre les virus et doit être officialisée dans une politique de sécurité du réseau.

Les produits antivirus sont basés sur l'hôte. Ces produits sont installés sur les ordinateurs et les serveurs pour détecter et éliminer les virus. Cependant, ils n'empêchent pas les virus de pénétrer sur le réseau. Un professionnel de la sécurité du réseau doit donc connaître les principaux virus et suivre les mises à jour de sécurité concernant les virus émergents.

Un autre moyen de limiter les menaces de malware est d'empêcher les fichiers malveillants d'entrer sur le réseau. Les périphériques de sécurité situés dans le périmètre du réseau peuvent identifier les fichiers malveillants connus en fonction de leurs indicateurs de compromission. Les fichiers peuvent être supprimés du flux de données entrant avant de provoquer un incident. Malheureusement, les hackers connaissent cette contre-mesure et modifient souvent leurs malwares au point d'échapper à la détection. Ces exploits entreront sur le réseau et échapperont également aux logiciels antivirus. Aucune technique d'atténuation ne peut être efficace à 100 %. Des incidents liés à la sécurité vont se produire.



# Lutter contre les vers

Les vers sont davantage basés sur le réseau que les virus. La réduction des vers nécessite de la diligence et de la coordination de la part des professionnels de la sécurité du réseau.
...
![[Pasted image 20250121103351.png]]
...
Les attaques par reconnaissance sont généralement précurseurs d'autres attaques visant à obtenir un accès non autorisé à un réseau ou à perturber ses fonctionnalités. Un professionnel de la sécurité du réseau peut détecter une attaque de reconnaissance en cours en recevant des notifications d'alarmes préconfigurées. Ces alarmes sont déclenchées lorsque certains paramètres sont dépassés, tels que le nombre de requêtes ICMP par seconde. Diverses technologies et équipements peuvent être utilisés pour surveiller ce type d'activité et générer une alarme. L'appliance de sécurité adaptative Cisco (ASA) assure la prévention des intrusions dans un périphérique autonome. En outre, les routeurs d'entreprise, tels que les routeurs à services intégrés Cisco (ISR), prennent en charge la prévention des intrusions basée sur le réseau avec des logiciels supplémentaires.

Les attaques par reconnaissance peuvent être atténuées de plusieurs manières, notamment:

- Mise en œuvre de l'authentification pour garantir un accès approprié.
- Utilisation du chiffrement pour rendre inutiles les attaques par détection de paquets.
- Utilisation d'outils anti-renifleur pour détecter les attaques par renifleur de paquets.
- Mise en œuvre d'une infrastructure commutée.
- Utiliser un pare-feu et un IPS.

Les outils logiciels et matériels anti-reniflés détectent les changements dans le temps de réponse des hôtes pour déterminer si ces derniers traitent plus de trafic que ne le laisserait supposer leur propre charge de trafic. Bien que cela n'élimine pas complètement la menace, dans le cadre d'un système global d'atténuation, cela peut réduire le nombre d'instances de menace.

Le chiffrement est également efficace pour limiter les attaques par détection de paquets. Si le trafic est chiffré, l'utilisation d'un détecteur de paquets est peu utile, car les données capturées ne sont pas lisibles.

Il est impossible de limiter l'analyse des ports, mais l'utilisation d'un système de prévention des intrusions (IPS) et d'un pare-feu peut limiter les informations pouvant être détectées avec un analyseur de ports. Les balayages ping peuvent être arrêtés si l'écho ICMP et la réponse à l'écho sont désactivés sur les routeurs Edge. Cependant, lorsque ces services sont désactivés, les données de diagnostic du réseau sont perdues. En outre, les analyses de ports peuvent être exécutées sans balayages ping complets. Les analyses prennent plus de temps, car les adresses IP inactives sont également analysées.

## Techniques de réduction des attaques par reconnaissance

![[Pasted image 20250121110907.png]]


# Limiter les attaques d'accès

Plusieurs techniques sont disponibles pour limiter les attaques. Il s'agit notamment de la sécurité renforcée par mot de passe, du principe de confiance minimale, de la cryptographie et de l'application de correctifs pour le système d'exploitation et les applications.

Un nombre surprenant d'attaques d'accès sont effectuées par simple tentative de devinette de mot de passe ou par attaque par dictionnaire de force brute contre des mots de passe. Pour éviter cela, créez et appliquez une politique d'authentification forte qui inclut:

- **Utilisez des mots de passe forts** - les mots de passe forts comportent au moins huit caractères et contiennent des lettres majuscules, des lettres minuscules, des chiffres et des caractères spéciaux.
- **Désactiver les comptes après un nombre spécifié d'échecs de connexion** - cette pratique permet d'éviter les tentatives continues de mot de passe.

Le réseau doit également être conçu selon le principe du minimum de confiance. Cela signifie que les systèmes ne doivent pas s'utiliser inutilement les uns les autres. Par exemple, si une entreprise dispose d'un serveur approuvé qui est utilisé par des périphériques non approuvés, tels que des serveurs web, le serveur approuvé ne doit pas approuver les périphériques non approuvés de manière inconditionnelle.

La cryptographie est un composant essentiel de tout réseau sécurisé moderne. Il est recommandé d'utiliser le chiffrement pour l'accès à distance à un réseau. Le trafic du protocole de routage doit également être chiffré. Plus le trafic est chiffré, moins les hackers ont d'opportunités d'intercepter les données avec des attaques de l'homme du milieu.

L'utilisation de protocoles d'authentification chiffrés ou hashs, ainsi qu'une politique de mots de passe forts, réduisent considérablement la probabilité d'attaques d'accès réussies.

Enfin, sensibilisez les employés aux risques de l'ingénierie sociale et élaborez des stratégies pour valider les identités par téléphone, par courriel ou en personne. L'authentification multifacteur (MFA) est de plus en plus courante. Dans cette approche, l'authentification nécessite au moins deux moyens de vérification indépendants. Par exemple, un mot de passe peut être combiné avec un code envoyé par SMS. Des logiciels ou des périphériques distincts peuvent être utilisés pour générer des jetons qui ne sont valides que pour une seule utilisation. Ces valeurs de jeton, lorsqu'elles sont fournies avec un mot de passe, fournissent une couche de sécurité supplémentaire qui empêche l'utilisation de mots de passe qui ont été devinés ou volés par des hackers.

En général, les attaques d'accès peuvent être détectées en examinant les journaux, l'utilisation de la bande passante et les charges de processus. La politique de sécurité du réseau doit spécifier que les journaux sont officiellement conservés pour tous les périphériques et serveurs du réseau. En examinant les journaux, le personnel de sécurité du réseau peut déterminer si un nombre inhabituel de tentatives de connexion a échoué.



# Atténuation des attaques DoS

L'un des premiers signes d'une attaque DoS est un grand nombre de plaintes d'utilisateurs concernant des ressources non disponibles ou des performances réseau anormalement lentes. Pour réduire le nombre d'attaques, un package logiciel d'utilisation du réseau doit être exécuté en permanence. L'analyse du comportement du réseau permet de détecter des modèles d'utilisation inhabituels qui indiquent qu'une attaque DoS est en cours. La politique de sécurité du réseau de l'entreprise doit prévoir un moyen de détecter les comportements inhabituels sur le réseau. Un graphique d'utilisation du réseau indiquant une activité inhabituelle peut également indiquer une attaque DoS.

Les attaques DoS peuvent faire partie d'une offensive plus vaste. Les attaques DoS peuvent entraîner des problèmes dans les segments de réseau des ordinateurs attaqués. Par exemple, la capacité de paquets par seconde d'un routeur entre Internet et un LAN peut être dépassée par une attaque, compromettant non seulement le système cible, mais aussi les périphériques réseau que le trafic doit traverser. Si l'attaque est menée à une échelle suffisamment grande, des zones géographiques entières de connectivité Internet peuvent être compromises.

Par le passé, de nombreuses attaques DoS provenaient d'adresses usurpées. Les routeurs et les commutateurs Cisco prennent en charge un certain nombre de technologies anti-usurpation d'identité, telles que la sécurité des ports, la surveillance DHCP (Protocole de configuration d'hôte dynamique), la protection de la source IP, l'inspection DAI (Protocole de résolution d'adresse dynamique) et les listes de contrôle d'accès (ACL).
...
Le WLAN (Wireless Local Area Network) est un réseau local sans fil qui permet à des appareils comme des ordinateurs, smartphones, tablettes ou imprimantes de se connecter entre eux et d’accéder à Internet sans avoir besoin de câbles. Voici comment il fonctionne :

### 1. **Le rôle du routeur ou du point d'accès**

Un **routeur sans fil** ou un **point d'accès sans fil (AP)** est le centre du WLAN. Il émet des ondes radio sur une certaine fréquence (généralement 2,4 GHz ou 5 GHz), créant un réseau auquel les appareils peuvent se connecter.

### 2. **Connexion des appareils**

Les appareils équipés de cartes réseau sans fil (Wi-Fi) détectent le signal du WLAN. Lorsqu’un appareil se connecte, il échange des données avec le point d'accès en utilisant ces ondes radio.

- **SSID (Service Set Identifier)** : Chaque réseau WLAN a un nom (SSID) qui permet aux utilisateurs d’identifier et de se connecter au bon réseau.

### 3. **Sécurité et chiffrement**

Pour protéger le réseau, plusieurs protocoles de sécurité peuvent être utilisés, comme :

- **WPA2 (Wi-Fi Protected Access 2)** : Un standard de sécurité courant qui chiffre les communications entre les appareils et le routeur pour empêcher l'interception des données.
- **Mot de passe Wi-Fi** : Le réseau est souvent protégé par un mot de passe pour limiter l'accès.

### 4. **Fonctionnement des fréquences radio**

Le WLAN utilise des **ondes radio** pour transmettre les données. Ces ondes voyagent dans l’air et permettent aux appareils de communiquer sans fil. Les appareils échangent des paquets de données avec le routeur via des canaux spécifiques sur les bandes de fréquence (2,4 GHz et 5 GHz).

- **2,4 GHz** : Plus de portée mais des vitesses plus lentes et plus de risques d'interférences avec d'autres appareils (comme des téléphones sans fil ou des fours à micro-ondes).
- **5 GHz** : Moins de portée mais des vitesses plus rapides et moins de risques d'interférences.

### 5. **Fonctionnement en réseau**

Une fois connectés, les appareils peuvent communiquer entre eux (comme envoyer des fichiers ou imprimer via une imprimante connectée), mais aussi accéder à Internet si le routeur est relié à une connexion Internet.

### 6. **Topologie et architecture**

Le WLAN peut adopter plusieurs configurations :

- **Infrastructure** : Le mode le plus courant où tous les appareils se connectent à un point d'accès central (le routeur).
- **Ad hoc** : Les appareils se connectent directement entre eux sans passer par un point d'accès, ce qui est utile pour des connexions temporaires ou des réseaux locaux limités.

### 7. **Qualité du signal et interférences**

La **portée** du WLAN dépend de la puissance du signal et des interférences dans l'environnement. Les murs, le mobilier et les appareils électroniques peuvent affecter la qualité du signal. Plus la distance entre l’appareil et le point d’accès est grande, plus la vitesse et la qualité de la connexion peuvent diminuer.

En résumé, le WLAN permet des connexions réseau sans fil grâce aux ondes radio, sécurisées par des protocoles comme WPA2, et gérées par un point d'accès ou un routeur sans fil.




# Réseaux locaux sans fil contre câblés

Les réseaux sans fil (WLAN) utilisent des fréquences radio au lieu de câbles au niveau de la couche physique et de la sous-couche MAC de la couche liaison de données. Les WLAN partagent une même origine avec les LAN Ethernet. L'IEEE a adopté l'ensemble des standards d'architecture de réseau informatique 802 LAN/MAN (réseau métropolitain). Les deux groupes de travail 802 dominants sont Ethernet 802.3, qui définit l'Ethernet pour les réseaux locaux câblés et 802.11 qui définit l'Ethernet pour les réseaux locaux sans fil. Il y a des différences importantes entre les deux.
...
Le tableau résume les différences entre les réseaux locaux sans fil et câblés.
...
| **Accès multimédia**       | évitement de collision                                                           | Détection de collision                 |
| **Disponibilité**          | toute personne disposant d'une carte réseau sans fil à portée d'un point d'accès | connexion par câble physique requise   |
| **Interférence de signal** | oui                                                                              | minimal                                |
| **Règlement**              | réglementations différentes selon les pays                                       | La norme IEEE dicte                    |
...
![[Pasted image 20250121114808.png]]
...
1. Écoute le canal pour voir s'il est inactif, ce qui signifie qu'il détecte qu'aucun autre trafic n'est actuellement sur le canal. Le canal est également appelé le transporteur.
...
4. Si le client sans fil ne reçoit pas de message CTS, il attend un temps aléatoire avant de redémarrer le processus.
5. Après avoir reçu le CTS, il transmet les données.
6. Toutes les transmissions sont reconnues. Si un client sans fil ne reçoit pas d'accusé de réception, il suppose qu'une collision s'est produite et redémarre le processus.



# Association entre le client sans fil et l'AP
...
![[Pasted image 20250121114956.png]]
...
# Mode de découverte passive et active

Les périphériques sans fil doivent détecter un point d'accès ou un routeur sans fil et s'y connecter. Les clients sans fil se connectent au point d'accès à l'aide d'un processus d'analyse (enquête). Ce processus peut être passif ou actif.



Mode passif

En mode passif, l'AP annonce ouvertement son service en envoyant périodiquement des trames de balise de diffusion contenant le SSID, les normes prises en charge et les paramètres de sécurité. Le but principal de la balise est de permettre aux clients sans fil d'apprendre quels réseaux et points d'accès sont disponibles dans une zone donnée. Esto permite que los clientes inalámbricos elijan qué red y AP usar.
...
![[Pasted image 20250121115214.png]]




# Dispositifs sans fil -AP, LWAP et WLC

Une mise en œuvre courante de réseau de données sans fil est de permettre à des périphériques de se connecter sans fil via un réseau local. En général, un réseau local sans fil nécessite des points d'accès sans fil et des clients dotés de cartes réseau sans fil. Comme illustré sur la figure, les routeurs sans fil domestiques et pour petite entreprise intègrent, dans un seul appareil, les fonctions d'un routeur, d'un commutateur et d'un point d'accès. Notez que, dans les petits réseaux, le routeur sans fil peut être le seul point d'accès du fait que la couverture sans fil n'est nécessaire que sur une petite zone. Dans des réseaux plus importants, il peut y avoir de nombreux points d'accès.


Toutes les fonctions de contrôle et de gestion des points d'accès d'un réseau peuvent être centralisées dans un contrôleur de réseau local sans fil (WLC). Lorsque vous utilisez un WLC, des points d'accès n'agissent plus de manière autonome, mais plutôt comme des points d'accès légers (LWAP). Les LWAP se contentent de transférer les données entre le réseau local sans fil et le WLC. Toutes les fonctions de gestion, telles que la définition du SSID et de l'authentification sont effectuées sur le WLC centralisé plutôt que sur chaque point d'accès individuel. Un avantage majeur de la centralisation des fonctions de gestion des points d'accès dans le WLC est la configuration et la surveillance simplifiées de nombreux points d'accès.



# Aperçu de la sécurité sans fil
...
# Attaque de l'homme du milieu (L’homme du milieu)
...
![[Pasted image 20250121134911.png]]
...
![[Pasted image 20250121134937.png]]
...
# Masquage du SSID et filtrage des adresses MAC
...
![[Pasted image 20250121135509.png]]
...
![[Pasted image 20250121135539.png]]


# Méthodes d'authentification originales 802.11
...
- **Authentification de système ouvert** - Tout client sans fil doit pouvoir se connecter facilement et ne doit être utilisé que dans des situations où la sécurité n'est pas un problème, comme dans les lieux offrant un accès gratuit à l'internet, comme les cafés, les hôtels et les zones reculées. Le client sans fil est responsable de la sécurité, comme l'utilisation d'un réseau privé virtuel (VPN) pour se connecter en toute sécurité. VPNs provide authentication and encryption services. Les VPN dépassent le cadre de cette rubrique.
- **Authentification par clépartagée** - Fournit des mécanismes, tels que WEP, WPA, WPA2 et WPA3 pour authentifier et crypter les données entre un client sans fil et un AP. Cependant, le mot de passe doit être pré-partagé entre les deux parties pour se connecter.

Le tableau suivant résume ces méthodes d'authentification.

![[Pasted image 20250121135610.png]]
...
| Méthode d'authentification     | La description                                                                                                                                                                                                                                                                                                            |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Wired Equivalent Privacy (WEP) | La spécification 802.11 originale conçue pour sécuriser les données à l'aide de la méthode de chiffrement Rivest Cipher 4 (RC4) avec une clé statique. Cependant, la clé ne change jamais lors de l'échange de paquets. Cela facilite le piratage. WEP n'est plus recommandé et ne doit jamais être utilisé.              |
| Wi-Fi Protected Access (WPA)   | Une norme Wi-Fi Alliance qui utilise WEP, mais sécurise les données avec l'algorithme de chiffrement TKIP (Protocole d'intégrité de clé temporelle) beaucoup plus puissant. TKIP change la clé pour chaque paquet, ce qui le rend beaucoup plus difficile à pirater.                                                      |
| WPA2                           | WPA2 est la norme actuelle de l'industrie pour la sécurisation des réseaux sans fil. Il utilise la norme de cryptage avancée (AES) pour le cryptage. AES est actuellement considéré comme le protocole de cryptage le plus puissant.                                                                                      |
...
- **Personnel -** Destiné aux réseaux domestiques ou de petits bureaux, les utilisateurs s'authentifient à l'aide d'une clé pré-partagée (PSK). Les clients sans fil s'authentifient auprès du routeur sans fil à l'aide d'un mot de passe pré-partagé. Aucun serveur d'authentification spécial n'est requis.
- **Enterprise -** Destiné aux réseaux d'entreprise mais nécessite un serveur d'authentification RADIUS (Remote Authentication Dial-In User Service). Bien que plus compliqué à configurer, il offre une sécurité supplémentaire. Le périphérique doit être authentifié par le serveur RADIUS, puis les utilisateurs doivent s'authentifier à l'aide de la norme 802.1X, qui utilise le protocole EAP (Extensible Authentication Protocol) pour l'authentification.

Dans la figure, l'administrateur configure le routeur sans fil avec l'authentification WPA2 Personal sur la bande 2.4 GHz.


![[Pasted image 20250121135829.png]]
...
![](https://contenthub.netacad.com/asset/netacad-media/media/ca958400-1c27-11ea-8212-a1efdf0f83be/assets/images/ca958400-1c27-11ea-af09-3b2e6521927c.png)
...
- **Adresse IP du serveur RADIUS -** Il s'agit de l'adresse joignable du serveur RADIUS.
- **Numéros de port UDP -** Ports UDP 1812 officiellement attribués pour l'authentification RADIUS et 1813 pour la comptabilité RADIUS, mais ils peuvent également fonctionner en utilisant les ports UDP 1645 et 1646, comme le montre la figure.
...
![](https://contenthub.netacad.com/asset/netacad-media/media/ca95d220-1c27-11ea-8212-a1efdf0f83be/assets/images/ca95d220-1c27-11ea-af09-3b2e6521927c.png)
...
![[Pasted image 20250121143758.png]]
...
![[Pasted image 20250121143939.png]]
...
![[Pasted image 20250121144022.png]]
...
![[Pasted image 20250121144103.png]]


# Descriptions des types de pare-feu

Il est important de comprendre les différents types de pare-feu et leurs capacités spécifiques afin d'utiliser le bon pare-feu pour chaque situation.
...
![[Pasted image 20250121144303.png]]
...
![[Pasted image 20250121144703.png]]
...
# Avantages et inconvénients des IDS et IPS

**Avantages et inconvénients des systèmes IDS**
...
**Avantages et inconvénients des systèmes IPS**

**Avantages d'IPS**

Advantages d'un IPS comprennent:
...
Il existe deux types principaux de systèmes de prévention des intrusions: IPS basé sur l'hôte et IPS basé sur le réseau.

**IPS basé sur l'hôte**

L'IPS basé sur l'hôte (HIPS) est un logiciel installé sur un hôte pour surveiller et analyser toute activité suspecte. L'un des avantages significatifs des HIPS est qu'ils peuvent surveiller et protéger les processus du système d'exploitation et les processus système critiques qui sont propres à l'hôte concerné. Grâce à une connaissance approfondie du système d'exploitation, HIPS peut surveiller les activités anormales et empêcher l'hôte d'exécuter des commandes qui ne correspondent pas au comportement type. Le comportement suspect ou malveillant peut comprendre les mises à jour non autorisées du registre, les modifications du répertoire système, l'exécution de programmes d'installation et les activités qui provoquent des débordements de tampon. Le trafic réseau peut être également surveillé pour empêcher l'hôte de participer à une attaque de déni de service (DoS) ou de faire partie d'une session FTP illicite.

HIPS peut être considéré comme une combinaison de logiciels antivirus, antimalware et de pare-feu. Associé avec un IPS en réseau, HIPS est un outil efficace pour fournir une protection supplémentaire pour l'hôte.

L'un des inconvénients de HIPS est qu'il n'intervient qu'au niveau local. Il n'a pas de vision complète du réseau ni de coordination des événements susceptibles de se produire sur le réseau. Pour être efficace dans un réseau, HIPS doit être installé sur chaque hôte et être pris en charge par chaque système d'exploitation.
...
Un IPS en réseau peut être implémenté en utilisant un dispositif IPS dédié ou non. Les implémentations d'IPS en réseau sont un élément essentiel de la prévention des intrusions. Il existe des solutions IDS/IPS d'hôte, mais elles doivent être intégrées avec une implémentation IPS réseau afin d'assurer une architecture de sécurité robuste.

Les capteurs détectent en temps réel toute activité malveillante et non autorisée, et peuvent prendre des mesures si nécessaire. Comme le montre la figure, les capteurs sont déployés à des points réseau désignés. Cela permet aux gestionnaires de sécurité de surveiller l'activité réseau pendant qu'elle a lieu, indépendamment de l'emplacement de la cible de l'attaque.

**Déploiement de capteurs IPS**

![[Pasted image 20250121160908.png]]


# Appareils de sécurité spécialisés

Il existe une variété d'appareils de sécurité spécialisés disponibles. En voici quelques exemples:

**AMP (Protection contre les programmes malveillants)**

Protection avancée contre les logiciels malveillants de Cisco (AMP) est une solution d'analyse et de protection contre les logiciels malveillants de niveau professionnel. Elle fournit aux entreprises une protection complète contre les logiciels malveillants avant, pendant et après une attaque:

- Avant une attaque, AMP renforce les défenses et protège contre les menaces connues et émergentes.
- Lors d'une attaque, AMP identifie et bloque les types de fichiers violant les politiques, les tentatives d'exploitation ainsi que les fichiers malveillants susceptibles d'infiltrer le réseau.
- Après une attaque, ou après l'inspection initiale du fichier, AMP va au-delà des capacités de détection ponctuelle et continue de surveiller et d'analyser l'ensemble des activités et du trafic sur les fichiers, qu'ils soient supprimés ou non, en recherchant des indices d'un comportement malveillant. Si un fichier inconnu ou précédemment classé inoffensif commence à avoir un comportement suspect, il est détecté par AMP qui alerte immédiatement les équipes en charge de la sécurité en leur fournissant un indicateur de compromission. Dès lors, AMP offre une visibilité sur l'origine et l'activité du malware ainsi que sur les systèmes menacés.

AMP accède aux connaissances de sécurité collectives du Cisco Talos Security Intelligence and Research Group. L'équipe Talos identifie et met en corrélation les attaques en temps réel grâce au réseau de détection le plus étendu au monde.



**WSA**

L'appliance Cisco de sécurisation du web (WSA) est une passerelle web sécurisée qui associe les meilleures protections pour permettre aux entreprises de répondre aux défis croissants de la sécurisation et du contrôle du trafic web. WSA protège le réseau en bloquant automatiquement les sites à risque et en testant les sites inconnus avant de permettre aux utilisateurs d'y accéder. WSA offre une protection contre les logiciels malveillants, permet la visibilité et le contrôle des applications, propose des rapports intelligents et assure la sécurité mobile.

Si WSA protège le réseau contre les intrusions de logiciels malveillants, il ne protège pas les utilisateurs qui souhaitent se connecter directement à Internet hors du réseau protégé, comme dans le cas d'un service de Wi-Fi public. Dans ce cas, le PC de l'utilisateur peut être infecté par des logiciels malveillants qui peuvent ensuite se propager à d'autres réseaux et appareils. La solution cloud Cisco de sécurité de la messagerie (CWS) est conçu pour protéger les PC des utilisateurs de ces types d'infections par des logiciels malveillants.

Associé à WSA, CWS assure une protection complète contre les logiciels malveillants et leur impact. La solution Cisco CWS met en œuvre une communication sécurisée vers et depuis l'Internet et fournit aux travailleurs à distance le même niveau de sécurité que des employés sur site, en cas d'utilisation d'un ordinateur portable géré par l'employeur. Cisco CWS intègre deux fonctions principales, à savoir le filtrage web et la sécurité web, et toutes deux sont accompagnées fonctionnalités de rapports complètes et centralisées.


**ESA**

L'appliance Cisco de sécurité de la messagerie (ESA) et la solution cloud Cisco de sécurité de la messagerie contribuent à atténuer les menaces reposant sur les e-mails. Cisco ESA défend les systèmes de messagerie stratégiques.

Cisco ESA est constamment mis à jour par des flux en temps réel de Cisco Talos, qui détecte et met en corrélation les menaces à l'aide d'un système de surveillance reposant sur une base de données mondiale.

Voici quelques-unes des principales fonctionnalités d'ESA:

- **Renseignements sur les menaces à l'échelle mondiale -** Cisco Talos offre une vue sur l'activité du trafic mondial 24 heures sur 24. Il analyse les anomalies, identifie les nouvelles menaces et surveille les tendances du trafic.
- **Blocage des spams -** Une défense multicouche combine une couche externe de filtrage basée sur la réputation de l'expéditeur et une couche interne de filtrage qui effectue une analyse approfondie du message.
- **Protection avancée contre les logiciels malveillants -** Comprend une AMP qui tire parti du vaste réseau d'intelligence de sécurité en nuage de Sourcefire. Il offre une protection sur toute la durée de l'attaque soit avant, pendant et après celle-ci.
- **Contrôle des messages sortants -** Contrôle les messages sortants afin de s'assurer que les messages importants sont conformes aux normes industrielles et sont protégés en transit.



# Contrôle du trafic avec ACLs
